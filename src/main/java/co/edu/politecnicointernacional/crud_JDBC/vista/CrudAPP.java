package co.edu.politecnicointernacional.crud_JDBC.vista;
	...
	public CrudAPP() {
		...
		btnCreate.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					Connection conexion = DriverManager.getConnection("jdbc:mariadb://localhost:3308/empresajpa",
							"root", "polint2023");
					System.out.println("conexion establecida");
					Statement comando = conexion.createStatement();
					System.out.println("buffer creado");
					...
					comando.executeUpdate("insert into empleados(nombreEmp,apellidoEmp,emailEmp) values ('" + nombre + "','"
							+ apellido + "','" + correo + "')");
		...
		txtIdEmp_R.setBounds(12, 44, 162, 30);
		pnRead.add(txtIdEmp_R);
		
		JTextArea txtResultEmp_R = new JTextArea();
		txtResultEmp_R.setBounds(192, 53, 207, 142);
		...
		btnRead.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					Connection conexionR = DriverManager.getConnection("jdbc:mariadb://localhost:3308/empresajpa",
							"root", "polint2023");
					System.out.println("conexion establecida");
					Statement comandoR = conexionR.createStatement();
					System.out.println("buffer creado");
					
					ResultSet consulta = comandoR.executeQuery("select *from empleados where idEmp= " + txtIdEmp_R.getText());
					
					if (consulta.next()) {
						txtResultEmp_R.append("Nombre: " + consulta.getString("nombreEmp")+ " ");
						txtResultEmp_R.append(consulta.getString("apellidoEmp")+ "\n");
						txtResultEmp_R.append("Email: " + consulta.getString("emailEmp"));
					}				
				} catch (SQLException ex) {
					System.out.println("error en SQl: " + ex);
				}
		...
		btnDelete.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				try {
					Connection conexionE = DriverManager.getConnection("jdbc:mariadb://localhost:3308/empresajpa",
							"root", "polint2023");
					System.out.println("conexion establecida");
					Statement comandoE = conexionE.createStatement();
					System.out.println("buffer creado");
					
					ResultSet eliminar = comandoE.executeQuery("delete from empleados where idEmp = " + txtIdEmp_R.getText());
					
					if (eliminar.next()) {
						lblResultado_D.setText("Nombre: " + eliminar.getString("idEmp")+ "Fue Eliminado ");
						//txtResultEmp_R.append(eliminar.getString("apellidoEmp")+ "\n");
						//txtResultEmp_R.append("Email: " + eliminar.getString("emailEmp"));
					}				
				} catch (SQLException ex) {
					System.out.println("error en SQl: " + ex);
				}
